<?php $currentLang = \App\Helpers\Lang::current(); ?>
<!DOCTYPE html>
<html lang="<?= $currentLang === 'ta' ? 'ta' : 'en' ?>">
<head>
  <meta charset="UTF-8">
  ...
  <?php if ($currentLang === 'ta'): ?>
  <style>
    body, .sidebar, .topbar, button, input, select, textarea {
...
<div class="app-layout">
  ...
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-brand">
      <h1><?= __('app_name') ?></h1>
      <p><?= htmlspecialchars(\App\DB::scalar('SELECT name FROM properties LIMIT 1') ?? __('common.property', [], 'Property')) ?></p>
    </div>

    <nav class="sidebar-nav" aria-label="Main navigation">
      <span class="nav-section-label"><?= __('nav.overview') ?></span>
      <?php
        $cur  = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);
        $_base = rtrim(\App\Helpers\UrlHelper::base(), '/');
        if ($_base !== '' && str_starts_with($cur, $_base)) {
            $cur = substr($cur, strlen($_base));
        }
        if ($cur === '') $cur = '/';
      ?>
      <a href="<?= url("/") ?>" class="nav-item <?= ($cur === '/' || $cur === '/dashboard') ? 'active' : '' ?>">
      ...
      <span class="nav-section-label"><?= __('nav.property') ?></span>
      <a href="<?= url("/rooms") ?>" class="nav-item <?= str_starts_with($cur, '/rooms') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/tenants") ?>" class="nav-item <?= str_starts_with($cur, '/tenants') ? 'active' : '' ?>">
      ...
      <span class="nav-section-label"><?= __('nav.finance') ?></span>
      <a href="<?= url("/dues") ?>" class="nav-item <?= str_starts_with($cur, '/dues') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/invoices") ?>" class="nav-item <?= ($cur === '/invoices') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/invoices/new") ?>" class="nav-item <?= str_starts_with($cur, '/invoices/new') ? 'active' : '' ?>" style="padding-left:42px;font-size:13px">
        + Generate Invoice
      </a>
      <a href="<?= url("/payments/new") ?>" class="nav-item <?= str_starts_with($cur, '/payments') ? 'active' : '' ?>">
        ...
        <?= __('nav.record_payment') ?>
      </a>
      <a href="<?= url("/reminders") ?>" class="nav-item <?= str_starts_with($cur, '/reminders') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/reports") ?>" class="nav-item <?= str_starts_with($cur, '/reports') ? 'active' : '' ?>">
        <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
        <?= __('nav.reports') ?>
      </a>

      <span class="nav-section-label"><?= __('nav.system') ?></span>
      <a href="<?= url("/audit") ?>" class="nav-item <?= str_starts_with($cur, '/audit') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/import") ?>" class="nav-item <?= str_starts_with($cur, '/import') ? 'active' : '' ?>">
      ...
      <a href="<?= url("/settings") ?>" class="nav-item <?= str_starts_with($cur, '/settings') ? 'active' : '' ?>">
    ...
    <!-- Language switcher -->
    <div style="padding:12px 16px;border-top:1px solid rgba(255,255,255,.1);margin-top:8px">
    ...
    <div class="sidebar-footer">
      ...
      <form action="<?= url("/logout") ?>" method="POST" style="margin:0">
        <input type="hidden" name="_csrf" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
        <button type="submit" class="logout-btn"><?= __('nav.sign_out') ?></button>
      </form>
    </div>
  </aside>
  ...
  <div class="main-wrap">
    <header class="topbar">
      <div class="d-flex align-center gap-12">
        ...
        <h2 class="topbar-title"><?= htmlspecialchars($pageTitle ?? __('app_name')) ?></h2>
      </div>
      <div class="topbar-actions">
        <a href="<?= url("/payments/new") ?>" class="btn btn-primary btn-sm"><?= __('topbar.record_payment') ?></a>
      </div>
    </header>
